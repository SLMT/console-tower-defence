...
// Initialize for receiving keyboard inputs. Must be called in the beginning.
void InitializeKeyInput();

// Wait for the key being pressed down in `sec` seconds.
int WaitForKeyDown(float sec);

// Get the character being pressed down lastly.
int GetKeyEventASCII();

// Get the key being pressed down lastly.
int GetKeyEventVirtual();

}  // namespace nthu_cs

#endif  // LIB_NTHU_CS_KB_INPUT_H_