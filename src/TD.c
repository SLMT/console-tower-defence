...
int frame[HEIGHT+2][WIDTH+2] = {0};   /* 外框 */
char plate[HEIGHT][WIDTH];  /* 畫面 */
...
int main(void)
{
   ...
   /* 顯示外框　*/
   putString(OFFSET_X - 2,OFFSET_Y-2,"Tower Defence 1.0",14,1);
	putString(OFFSET_X + WIDTH*2 - 10,OFFSET_Y-2,"Made by SLMT",14,3);
   for (i = 0; i < HEIGHT+2; i++) {
   	for (j = 0; j < WIDTH+2; j++) {
      	if (frame[i][j] == 1) {
   ...
   initializeKeyInput();
	...
	while(IsEnding != 1) {
		...
		if(IsMain != 1) { 
			setMainMenu(&m);
			IsMain = 1;
			getImage(); /*  開始讀入預先製作的imageMenu.txt檔 */
			genJune(&J[0]);
		}
		
		/* 如果囧人停下來，而且還沒產生超過 6 個人的話，就產生新的囧人 */
      	...
      	vk=getKeyEventVirtual();
			...
			switch (vk)	{
				case VK_UP:
					...
					break;
				case VK_DOWN:
   	      	if(m.cursor != m.num_options-1) { /* 如果游標在最後一個，則不做任何事 */ 
	            	m.state[m.cursor]=0;
	            	m.cursor++;
   	         	m.state[m.cursor]=1;
					}
					break;
				case VK_RETURN:
					switch (m.cursor) {     /* 在不同位置按下 ENTER 有不同反應 */ 
						case 0:
							if(sin_game() == 1)
								IsEnding = 1;
							num = 0;
		...
		if((float)(clock()-t_start) >= 0.3*CLOCKS_PER_SEC) {
			...
			if(J[num].x >= (18+num*3))
				...
				J[num].state = 2; /* 把囧人的狀態設為 2 代表「停止」 */
			t_start = clock();
		}
	} /* while(IsEnding != 1) 結尾 */ 
	...
	while(1) {
		...
		if(i==0)
			...
			putString(24+OFFSET_X,18+OFFSET_Y,"請按 Enter 離開遊戲...",12,0);
		drawCmdWindow();
		i= !i;
	}
...
void setMainMenu(Menu *m)
{
	/* 將預先做好的部分開頭畫面 MainMenu.txt 存入 plate 中 */
	FILE *in;
	char ch;
	int i,j;
   ...
   strcpy(m->text[0], "  單人模式 ONE PLAYER");
   strcpy(m->text[1], "  雙人模式 TWO PLAYERS");
   strcpy(m->text[2], "  遊戲介紹 INTRODRCTION");
   strcpy(m->text[3], "  作者介紹 ABOUT MAKER");
   strcpy(m->text[4], "  排行榜 RANKING LIST");
   strcpy(m->text[5], "  離開遊戲 EXIT");
   strcpy(m->alt_text[0], "->單人模式 ONE PLAYER");
   strcpy(m->alt_text[1], "->雙人模式 TWO PLAYERS");
   strcpy(m->alt_text[2], "->遊戲介紹 INTRODRCTION");
   strcpy(m->alt_text[3], "->作者介紹 ABOUT MAKER");
   strcpy(m->alt_text[4], "->排行榜 RANKING LIST");
   strcpy(m->alt_text[5], "->離開遊戲 EXIT");
}
...
{	
	...
	putString(16+OFFSET_X,2+OFFSET_Y,"囧人：",10,0);
	putString(20+OFFSET_X,3+OFFSET_Y,"我....我投降！！",10,0);
	putString(20+OFFSET_X,4+OFFSET_Y,"我想離開了....",10,0);
}
...
void showJune(June *j)
{
	...
	for (i=0; i<3; i++) {
      for (k=0; k<3; k++) {
			if((j->x+k < 35) && (j->x+k >= 0) && (j->y+i < 20))     /* 防止囧人超出外框 */
	         switch (j->shape[i][k]) {
   	         case 1:
