...
int NoColorTS[1][18] = {
		0xa2, 0xa8, 0xa1, 0xbd, 0xa2, 0xa9, 
		0xa1, 0xbd, 0xa1, 0xb7, 0xa1, 0xbd, 
		0xa1, 0xbd, 0xa1, 0xbc, 0xa1, 0xbd };

/* 圖片陣列區 Ending*/

/* 這裡包含了一些可以讀取讀取外部檔案的函式 */
/* 然後將這些外部檔案轉成圖片，並且顯示在螢幕上 */
	...
	for(i=0;i<IM_HEIGHT;i++) {
		for(j=0;j<IM_WIDTH;j++) {
			if(image[num][i][j] == 'U') {
				...
				ch1=0xA1;  ch2=0xB4;  fg_color=14;
			} else if(image[num][i][j] == 'e') {
				ch1=0xA2;  ch2=0x65;  fg_color=12;
			} else if(image[num][i][j] == 'v') {
				ch1=0xA2;  ch2=0x63;  fg_color=12;
			} else if(image[num][i][j] == 'Q') {
				ch1=0xCA;  ch2=0xA8;  fg_color=11;
