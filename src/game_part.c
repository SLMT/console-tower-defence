...
extern int sin_game(void);    /* 遊戲會回傳一個值，告訴程式玩家是否要立刻離開遊戲 */
...
int sin_game(void)
{
	...
	int TS[3][6];
	int noMoney=0;
	int UG=0;  /* 檢查升級是否可行的變數 */
	int showTM=0;
	
	char toString[100];  /* 把數字變成字串 */
	
	Menu exit,TM;
	Choice CI;
	Moster *First_m; /* 用指標指向第一隻怪物 */
	Tower *First_t;
	...
	while(IsEnding != 1) {
		...
		/* 主要畫面顯示區 */
			switch (vk) {
      		case VK_ESCAPE:  /* 如果按下 ESC 鍵就跳出離開選單 */ 
					Exit=1;    break;
				case VK_UP:
					if(showTM == 0) {
						if(CI.y > 0)
		...
		if((float)(clock()-MSMoveTimer) > CLOCKS_PER_SEC * MSMoveTime) {
			...
			if(nextTime == 0) {
				moveMoster(First_m);  /* 讓怪物「們」往前移動一步 */
				moster_step++;
		...
		showMap(map_x,map_y); /* 顯示地圖 */
		showMoster(First_m,map_x,map_y);
		showTower(First_t,map_x,map_y);	
		EndPoint(map_x,map_y,0); /* 顯示終點 */
		
		/* 檢驗第一隻是否到達終點( 要先檢查看看 First_m 是否為 NULL ) */
		if( First_m != NULL )
			if( First_m->x > 46 ) {
		...
		sprintf(toString,"Main Tower HP：%3d",HP);  /* 血量 */
		putString(0 + OFFSET_X,0 + OFFSET_Y,toString,12,7);
		
		sprintf(toString,"$：%5d",money);  /* 金錢 */
		...
		if(nextTime != 0) {
			sprintf(toString,"距離第%2d波攻擊還有..%2d",wave,nextTime);  /* 分數 */
			putString(20 + OFFSET_X,HEIGHT-1 + OFFSET_Y,toString,12,11);
		}
		
		if(noMoney == 1) {  /* 錢不夠時提醒一下 */
			putString(22+OFFSET_X,8+OFFSET_Y,"                    ",14,5);
			putString(22+OFFSET_X,9+OFFSET_Y,"      餘額不足      ",14,5);
		...
		/* 選單區 */
		} else if(Exit == 1) { /* 初始化離開選單 */
			setExitMenu(&exit);
			Exit=2;
		} else if(Exit == 2) { /* 顯示離開選單 */
			showExitMenu(&exit);
			switch (vk)	{
				case VK_UP:
					if(exit.cursor != 0) {   /* 如果游標在第一個，則不做任何事 */
					...
					break;
				case VK_DOWN:
   	      	if(exit.cursor != exit.num_options-1) { /* 如果游標在最後一個，則不做任何事 */ 
	            	exit.state[exit.cursor]=0;
	            	exit.cursor++;
   	         	exit.state[exit.cursor]=1;
					}
					break;
				case VK_RETURN:
					switch (exit.cursor) {     /* 在不同位置按下 ENTER 有不同反應 */ 
						case 0:
							clearScreen();
							Exit=0; break;
						case 1:
							return 0;
						case 2:
							return 1; /* 若選擇 exit 則直接 return 1 離開遊戲 */
	...
	while(1) {
		...
		if(i==0)
			...
			putString(24+OFFSET_X,12+OFFSET_Y,"請按 Enter 繼續...",11,5);
		drawCmdWindow();
		i= !i;
	}
...
{	
   ...
   strcpy(m->text[0], "  繼續遊戲 Continue");
   strcpy(m->text[1], "  回到主選單 MainMenu");
   strcpy(m->text[2], "  離開遊戲 Exit");
   strcpy(m->alt_text[0], "->繼續遊戲 Continue");
   strcpy(m->alt_text[1], "->回到主選單 MainMenu");
   strcpy(m->alt_text[2], "->離開遊戲 Exit");
}
...
Moster* genMoster(Moster *Fm,int mos_num,int op_x,int op_y,int level)
{
	...
	newPtr = (Moster*)malloc(sizeof(Moster));  /* 先空出一個 Moster型別 的記憶體位置 */
	if (newPtr != NULL) { 
		...
		newPtr->value = level*2 + mos_num - 2;  /* 怪物價值為 level*2 加上種類減二 */
		newPtr->nextPtr = NULL;
		moster_shape(newPtr->kind,newPtr->state,newPtr->shape); /* 取得形狀 */
		
		/* 如果沒有怪物，就直接把 Fm 指向新怪物 */
		if(Fm == NULL) {
			Fm = newPtr;
			return Fm;
		} else {
			...
			while (curPtr != NULL) {
				prePtr = curPtr;
				curPtr = curPtr->nextPtr;   /* 抓出下一個指標，然後再檢查 */
			}
...
void showMoster(Moster *m,int map_x,int map_y)
{
	...
	while(m != NULL) {
		show_x = m->x - map_x;  /* 怪物顯示時的座標必須用怪物座標減去地圖顯示座標 */
		show_y = m->y - map_y;
		for (i=0; i<3; i++)
      	for (j=0; j<3; j++)
...
void moveMoster(Moster *m)
{
	...
	while(m != NULL) {
		...
		if(map[next_y][next_x*2] == 0xa1 && m->direct == 1) { /* 向右遇到牆壁的話 */
			...
			if(map[next_y][next_x*2] != 0xa1) /* 看看左邊是否是牆壁 */ 
				m->direct--; /* 左轉 */ 
			next_x = m->x+1 + head_x[2]*2;
			next_y = m->y+1 + head_y[2]*2;
			if(map[next_y][next_x*2] != 0xa1) /* 看看右邊是否是牆壁 */ 
				m->direct++; /* 右轉 */ 
		...
		m->state = !(m->state);  /* 每移動一步就改變姿勢 */
		moster_shape(m->kind,m->state,m->shape); /* 取得形狀 */
		
		m = m->nextPtr; /* 取得下一隻的位置 */
	}
}
...
Moster* deleteMoster(Moster *Fm,int num)
{
	...
	if(num == 1) {
		...
		tempPtr = curPtr; /* 為了等一下釋放記憶體，要先記下節點的位置 */
		prePtr->nextPtr = curPtr->nextPtr;
		free( tempPtr );   /* 把要刪除的節點記憶體釋放掉 */ 
	}
	return Fm;
}
	...
	while( t != NULL ) {
		while( curMP != NULL ) {
			...
			(float)clock() - t->timer > CLOCKS_PER_SEC/quickMove ) {
				/* 利用商高定理檢驗敵人是否在範圍內 ， 以及距離上次攻擊使否達到必要時間 */
				/* 如果在範圍內，塔就開始攻擊 */
				
				if((t->atkValue - m->deValue) <= 0) {/* 確認攻擊是否小於怪物防禦 */
					if(curMP->hp < (4 - m->kind)*1 )
						curMP->hp = 0;
					else
						curMP->hp -= (4 - m->kind)*1;
				}
				else if(curMP->hp > (t->atkValue - m->deValue)) /* 確認血量大於減少的血 */
...
Moster* ColorAndDead(Moster *Fm)
{
	...
	while( m != NULL) {
		...
		if(m->hp == 0) {
			...
			num--; /* 因為刪掉一隻怪物會 num 會減少，這樣 num 的數量就不正確了 */
			money+=(10*m->value);
			score+=(5*m->value);
		}
...
Tower* createTower(Tower *FT,int kind,int op_x,int op_y)
{
	...
	newPtr = (Tower*)malloc(sizeof(Tower));  /* 先空出一個 Tower型別 的記憶體位置 */
	if (newPtr != NULL) {  /* 確認真的有「確實空出」一塊能用的記憶體 */
		/* 初始化新塔的資訊 */
		newPtr->x = op_x;
		newPtr->y = op_y;
		newPtr->kind = kind;
		...
		newPtr->atkValue = 15;
		newPtr->timer = clock();
		newPtr->nextPtr = NULL;   /* 指標則先暫時不讓它指向任何東西 */
		tower_shape(newPtr->kind,newPtr->shape); /* 存入形狀 */
		
		prePtr = NULL;
		curPtr = FT;     /* 把最初的指標 np 存進 curPtr */
...
void showTower(Tower *FT,int map_x,int map_y)
{
	...
	while(FT != NULL) {
		show_x = FT->x - map_x;  /* 怪物顯示時的座標必須用怪物座標減去地圖顯示座標 */
		show_y = FT->y - map_y;
		/* 不同等級顯示不同顏色的磚塊 */
		switch (FT->level) {
         case 1:
...
/************ 檢查即將建造的 Tower 是否於合法位置 ************/
int TowerCheck(int cu_x,int cu_y,Tower *FT)
{
	int i,j;
	
	/* 檢查即將建造的 Tower 是否與已存在的重疊 */	
	while( FT != NULL ) {
...
int setTowerMenu(Tower *FT,Menu *m,int x,int y)
{
   ...
   strcpy(m->text[0], "  升級 Upgrade");
   strcpy(m->text[1], "  出售 Sale");
   strcpy(m->text[2], "  關閉 Close");
   strcpy(m->alt_text[0], "->升級 Upgrade");
   strcpy(m->alt_text[1], "->出售 Sale");
   strcpy(m->alt_text[2], "->關閉 Close");
   ...
   while(FT != NULL) {
	...
	if(FT != NULL) {
		if(FT->kind == 1)
			sprintf(m->infom[0],"Kind: 砲塔");
		else if(FT->kind == 1)
			sprintf(m->infom[0],"Kind: 狙擊塔");
		else if(FT->kind == 1)
			sprintf(m->infom[0],"Kind: 機槍塔");
...
void EndPoint(int map_x,int map_y,int state)
{
	...
	if(EndPointTimer == 14) /* 每換兩次(7秒一次)顏色該值歸零 */
		EndPointTimer=0;
	...
	show_x = END_POINT_X - map_x;  /* 出生點顯示時的座標必須用怪物座標減去地圖顯示座標 */
